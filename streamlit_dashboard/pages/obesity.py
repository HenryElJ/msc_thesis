...
from tabs import obesity_introduction, obesity_data, obesity_model

# Import all files
with open("h1n1/h1n1_explanations_output.pickle", "rb") as file:
...
with data_tab:
    ...
    if st.session_state.ask_ai[1]:
        exec(f'''{st.session_state.tab}_col, {st.session_state.tab}_ai = st.columns([0.5, 0.5]);\nwith {st.session_state.tab}_col:\n\t{generate_tab(obesity_data)}''')
        exec(add_chatbox_col(st.session_state.tab))
    else:
        exec(generate_tab(obesity_data))
...
with model_tab:
    ...
    if st.session_state.ask_ai[2]:
        exec(f'''{st.session_state.tab}_col, {st.session_state.tab}_ai = st.columns([0.5, 0.5]);\nwith {st.session_state.tab}_col:\n\t{generate_tab(obesity_model)}''')
        exec(add_chatbox_col(st.session_state.tab))
    else:
        exec(generate_tab(obesity_model))
