...
from initialise import read_pkl
from tabs import tabs

# Page configurations
st.set_page_config(layout = "wide")
# current_page = st_javascript("await fetch('').then(r => window.parent.location.href)").rsplit('/',1)[1]

screen_height = ScreenData().st_screen_data(key = "screen_stats_height")["innerHeight"]
screen_width = ScreenData().st_screen_data(key = "screen_stats_width")["innerWidth"]
dict_padding = 220; viz_padding = 130; eda_padding = 225; chat_padding = 270; response_height = screen_height - chat_padding - 105

try:
    if ("current_page" not in st.session_state) | (st.session_state.current_page != "h1n1"):
        st.session_state.current_page = "h1n1"
except:
    st.session_state.current_page = "h1n1"
...
# Import all files
eda_output = read_pkl(f"{st.session_state.current_page}/eda_output.pickle")
# modelling_output = read_pkl(f"{current_page}/modelling_output.pickle")
explanations_output = read_pkl(f"{st.session_state.current_page}/explanations_output.pickle")

st.logo(f"images/llm_icons/{st.session_state.button_selection}.png", size = "small")

user_avatar = ":material/cognition:"; ai_avatar = f"images/llm_icons/{st.session_state.model_selection}.png"

st.sidebar.markdown('''
                    <a href = "http://localhost:8501/" target = "_self" class = "convex-link">
                    <div style="display: flex; align-items: center; gap: 10px; max-width: fit-content; margin-inline: auto;">
                    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Rounded:opsz,wght,FILL,GRAD@24,400,0,0" />
                    <span class="material-symbols-rounded" style="font-size: 60px; color: #B2A8D3;">cognition</span>
                    <h1 style="font-size: 2em; margin: 0; width: fit-content;">ConveX</h1>
                    </div>
                    </a>
...
st.sidebar.markdown('''
                    ...
                    <div style="display: flex; align-items: center; gap: 10px; max-width: fit-content; margin-inline: auto">
                    <center>
                    <h3 style="font-size: 1em; margin: 0; width: fit-content;">Your personalised Conversational eXplainer</h3>
                    </center>
                    </div>
                    </a>
                    ''', unsafe_allow_html = True)
...
st.sidebar.divider()
st.sidebar.markdown("<center><h5>Conversational Explanations (ConveX) ©, 2025</h5></center>", unsafe_allow_html = True)
...
# Tab configuration
introduction_tab, data_tab, model_tab, explanations_tab = st.tabs(["\u2001" * 10 + x for x in [":microbe: Introduction", ":bar_chart: Data", ":robot_face: Model", ":sparkles: Explanations"]])
# introduction_tab, data_tab, model_tab, explanations_tab = st.tabs([("\u2001" * (30 - len(x + y))) + x + y for x, y in zip([":microbe:", ":bar_chart:", ":robot_face:", ":sparkles:"], [" Introduction", " Data", " Model", " Explanations"])])
# INTRODUCTION
...
with data_tab:
    ...
    if st.session_state.ask_ai[1]:
        exec(f'''{st.session_state.tab}_col, {st.session_state.tab}_ai = st.columns([0.5, 0.5]);\nwith {st.session_state.tab}_col:\n\t{generate_tab(tabs[st.session_state.current_page]["data"])}''')
        exec(add_chatbox_col(st.session_state.tab))
    else:
        exec(generate_tab(tabs[st.session_state.current_page]["data"]))
...
with model_tab:
    ...
    if st.session_state.ask_ai[2]:
        exec(f'''{st.session_state.tab}_col, {st.session_state.tab}_ai = st.columns([0.5, 0.5]);\nwith {st.session_state.tab}_col:\n\t{generate_tab(tabs[st.session_state.current_page]["model"])}''')
        exec(add_chatbox_col(st.session_state.tab))
    else:
        exec(generate_tab(tabs[st.session_state.current_page]["model"]))
    ...
    with chatbox_col:
        ...
        if query := st.chat_input("Ask me a question!", accept_file = True, file_type = accepted_file_types):
            with chat_container:
                ...
                user_avatar = ":material/cognition:"; ai_avatar = f"images/llm_icons/{model_selection}.png"
...
st.markdown(f'''<style>
            ...
            /* Tab height */
            .st-al.st-as.st-bh.st-bd.st-fh.st-fi.st-fj.st-fk.st-fl.st-fm.st-fn.st-fo.st-fp {{height: 0rem}}
            /* Tab body padding */
            .st-bd.st-c0.st-dv.st-c8.st-c6.st-c7 {{padding-top: 0rem}}
            /* Floating button */
            .st-emotion-cache-37cg8s.eacrzsi1 {{background-image: linear-gradient(to bottom right, #B2A8D3, #A7C4E5); border-color: white}}
            .st-emotion-cache-37cg8s.eacrzsi1:hover {{background-image: linear-gradient(to bottom right, #A7C4E5, #B2A8D3); border-color: white}}
            </style>''', unsafe_allow_html = True)

# opacity: 0.5 !important;
# .st-emotion-cache-8atqhb.e1mlolmg0 {{height: 0}}

# <img src="data:image/png;base64,{logo}" alt="Your Image" style="width: 100px; height: 100px;">
