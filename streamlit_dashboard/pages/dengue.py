...
from tabs import dengue_introduction, dengue_data, dengue_model
...
with data_tab:
    ...
    if st.session_state.ask_ai[1]:
        exec(f'''{st.session_state.tab}_col, {st.session_state.tab}_ai = st.columns([0.5, 0.5]);\nwith {st.session_state.tab}_col:\n\t{generate_tab(dengue_data)}''')
        exec(add_chatbox_col(st.session_state.tab))
    else:
        exec(generate_tab(dengue_data))
...
with model_tab:
    ...
    if st.session_state.ask_ai[2]:
        exec(f'''{st.session_state.tab}_col, {st.session_state.tab}_ai = st.columns([0.5, 0.5]);\nwith {st.session_state.tab}_col:\n\t{generate_tab(dengue_model)}''')
        exec(add_chatbox_col(st.session_state.tab))
    else:
        exec(generate_tab(dengue_model))
